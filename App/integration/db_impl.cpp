...
namespace leveldb {
  ...
  Status DBImpl::FinishCompactionOutputFile1(CompactionState* compact) {
    assert(compact != NULL);
    assert(compact->outfile != NULL);
    assert(compact->builder != NULL);
...
#if 1
  Status DBImpl::SU_flush_data(Slice &key, Slice &value, CompactionState *compact, std::string *current_user_key, 
      bool *has_current_user_key, SequenceNumber *last_sequence_for_key) {
    Status status;
    int i=0;
    ...
    if (compact->compaction->ShouldStopBefore(key) &&
        compact->builder != NULL) {
      status = FinishCompactionOutputFile1(compact);
      if (!status.ok()) {
        return status;
      }
    }
  ...
  Status DBImpl::SU_compact(CompactionState* compact, int n_ways) {
